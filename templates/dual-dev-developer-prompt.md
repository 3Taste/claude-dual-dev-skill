...
**【语言约束】全程使用中文进行对话、提问、解释和说明。代码、命令、文件路径保持原样，其余所有输出必须是中文。**
...
- **遵守当前项目的所有规范**：优先读取 `CLAUDE.md` 和 `.claude/rules/` 下的规则文件，按其中定义的约定实现
...
若项目有测试命令（参考 `CLAUDE.md` 或 `{{BUILD_COMMAND}}` 对应的测试命令），运行相关测试。无测试跳过。
